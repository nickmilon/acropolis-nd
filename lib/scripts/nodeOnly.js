...
class ConLog extends Console {
  constructor(
    ...
    ConLog.levels = { silly: 1, trace: 2, debug: 3, info: 4, log: 5, warn: 6, error: 7, silent: 10 };
  ...
  _pass(level) { return ((this._level < 10) && this._validateLevel(level) >= this._level); } // short sircuit silent for efficiency
  ...
  error(data, ...args) { return this._print('debug', data, ...args); }
