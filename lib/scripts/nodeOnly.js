 ...
 */
const inspectIt = (obj, logger = null, commend = '', { showHidden = false, depth = 10, colors = true, breakLength = 120, maxStringLength = null } = {}) => {
  if (!logger) { return; }
  const inspObj = inspect(obj, { showHidden, depth, colors, breakLength, maxStringLength });
  logger.info(`-------------------: ${commend}\n${inspObj}\n--------------------`);
};
...
const consolDummy = {
  ...
  error: () => true,
  groupCollapsed: () => true,
  inspectIt: () => true, // doesn't exist in console, added for compatibility with ConLog
};
...
class ConLog extends Console {
  static levels = {
    silly: 1, trace: 2, debug: 3, info: 4, log: 5, warn: 6, error: 7, silent: 10,
  };
