...
/**
 * PageScroll constructor options
 * @typedef {Object} PageScrollOpts
 * @property {Function=} [fnNext = (row) => row]  - a function that returns a pointer that clients sharing same convention can use tp request next page
 * @property {Function=} [fnPrev = (row) => row]  - a function that returns a pointer that clients sharing same convention can use tp request previous page
 * @property {Integer=} [50] vectorDefault        - a vector (signed integer) to be used a default if one is not provided by subsequent scrolling calls
 * @property {Bool=} [true] resultsDesc           - used only when vector is a negative integer. When clients are scrolling backwards usually results are
 * sorted in descending order as referred by fnPrev use false to override this convention
 */

/**
 * paging Object
 * @typedef {object} pageObj
 * @property {Integer} position   - Where we stand (results Array relative to whole record set) 1=reached start -I reached start 0 somewhere in the middle
